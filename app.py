...
"""
    return summary + "\n\n*(Note: Generated by Fallback Logic because the AI Connection Failed. See error details above.)*"
...
def perform_audit(url, api_key):
    ...
    # Updated Model List (Most reliable first)
    models = [
        "meta-llama/llama-3.2-11b-vision-instruct:free", # Usually most stable
    ...
    try:
        ...
        Based on the content above, classify the business.
        (NOTE: If the content mentions "services", "solutions", or "booking", it is a SERVICE, even if it uses WooCommerce).
        ...
        for model in models:
            try:
                ...
                if ai_summary: 
                    status_msg.text("AI Report Generated Successfully!")
                    break 
        ...
        if not ai_summary:
            # DEBUG: Show the user WHY it failed
            st.error(f"⚠️ AI Connection Failed. Showing Fallback Report. Error details: {last_error}")
